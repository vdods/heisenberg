...
def apply_along_axes (func, input_axis_v, input_array, *args, output_axis_v=None, func_output_shape=None, **kwargs):
    ...
    assert __is_subsequence_of_nondecreasing_sequence(input_axis_v, A), 'input_axis_v (which is {0}), must be a nonempty subsequence of (0,1,...,{1})'.format(input_axis_v, len(input_array.shape)-1)

    is_not_input_axis_v = np.array([axis_index not in input_axis_v for axis_index in A])
    non_input_axis_v = np.array([axis_index for axis_index in A if is_not_input_axis_v[axis_index]])
    input_iterator_v = [range(input_array.shape[axis_index]) if is_not_input_axis_v[axis_index] else [slice(None)] for axis_index in A]
    # print('is_not_input_axis_v = {0}'.format(is_not_input_axis_v))
    # print('non_input_axis_v = {0}'.format(non_input_axis_v))
    ...
    if func_output_shape is None:
        func_output_shape = np.shape(func(input_array[tuple(0 if axis_index not in input_axis_v else slice(None) for axis_index in A)], *args, **kwargs))
    # print('func_output_shape = {0}'.format(func_output_shape))

    # B = np.arange(N-len(input_axis_v)+len(func_output_shape))
    B = np.arange(len(non_input_axis_v)+len(func_output_shape))
    # print('B = {0}'.format(B))

    # If output_axis_v is None, then it will be assumed to be the trailing axes of the output.
    if output_axis_v is None:
        output_axis_v = np.arange(len(B)-len(func_output_shape), len(B))
    # print('input_axis_v = {0}'.format(input_axis_v))
    ...
    assert __is_subsequence_of_nondecreasing_sequence(output_axis_v, B), 'output_axis_v (which is {0}), must be a subsequence of [0,...,{1}) (right endpoint excluded)'.format(output_axis_v, len(B))

    is_not_output_axis_v = [axis_index not in output_axis_v for axis_index in B]
    ...
    output_axis_inv_v = __index_map_inverse(output_axis_v, len(B))
    # print('output_axis_inv_v = {0}'.format(output_axis_inv_v))
    # print('__compose_index_maps(non_input_axis_v, non_output_axis_inv_v) = {0}'.format(__compose_index_maps(non_input_axis_v, non_output_axis_inv_v)))
    output_iterator_v = [range(input_array.shape[non_input_axis_v[non_output_axis_inv_v[axis_index]]]) if is_not_output_axis_v[axis_index] else [slice(None)] for axis_index in B]
    # print('output_iterator_v = {0}'.format(output_iterator_v))

    retval = np.ndarray(tuple(input_array.shape[non_input_axis_v[non_output_axis_inv_v[axis_index]]] if is_not_output_axis_v[axis_index] else func_output_shape[output_axis_inv_v[axis_index]] for axis_index in B), dtype=input_array.dtype)
...
def __test__apply_along_axes ():
    ...
    for input_i0 in range(3):
        for input_i1 in range(input_i0+1,4):
            for output_i0 in range(3):
                for output_i1 in range(output_i0+1,4):
                    # Compute the result.  The multi-slice across the output axes should be a symmetric matrix.
                    result = apply_along_axes(symmetric_square, (input_i0,input_i1), a, output_axis_v=(output_i0,output_i1))
                    # Figure out which indices correspond to the input axes; call these result_non_output_indices.
                    all_indices = (0,1,2,3)
                    output_indices = (output_i0,output_i1)
                    result_non_output_indices = sorted(list(frozenset(all_indices) - frozenset(output_indices)))
                    # print('output_indices = {0}, result_non_output_indices = {1}'.format(output_indices, result_non_output_indices))
                    assert len(result_non_output_indices) == 2
                    # Take all multi-slices and verify symmetry.
                    for check_i0,check_i1 in itertools.product(range(result.shape[result_non_output_indices[0]]), range(result.shape[result_non_output_indices[1]])):
                        # Construct the multi-slice to take.
                        multislice = [slice(None) for _ in range(4)]
                        multislice[result_non_output_indices[0]] = check_i0
                        multislice[result_non_output_indices[1]] = check_i1
